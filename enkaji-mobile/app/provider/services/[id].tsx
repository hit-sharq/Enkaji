...
export default function EditServiceScreen() {
  ...
  const loadService = async () => {
    try {
      const response = await api.getService(params.id!)
      if (response.service) {
        const service = response.service as Service
        setForm({
          name: service.name,
          ...
          location: service.city && service.county ? `${service.city}, ${service.county}` : (service.location || ''),
          address: service.address || '',
          images: service.images?.join('\n') || '',
          tags: (service as any).tags?.join(', ') || '',
        })
        setSelectedCategory(service.category)
        setPriceType((service as any).priceType || 'FIXED')
        setSelectedDays((service as any).availableDays || [])
        setStartTime((service as any).startTime || '09:00')
        setEndTime((service as any).endTime || '17:00')
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await api.updateService(params.id!, serviceData)
      ...
      Alert.alert('Error', error?.response?.data?.error || 'Failed to update service. Please try again.')
    } finally {
      setIsSaving(false)
    }
  }
