    ...
    try {
      ...
      if (response.success || response.id) {
        Alert.alert(
          ...
          [{ text: 'Back to Products', onPress: () => router.replace('/seller/products') }]
        )
      } else {
        Alert.alert('Error', response.error || 'Failed to update product')
      }
    } catch (error: any) {
      Alert.alert('Error', error?.response?.data?.error || 'Failed to update product. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
            ...
            {form.comparePrice && parseFloat(form.comparePrice) > parseFloat(form.price || '0') && (
              <View style={styles.discountHint}>
                ...
                <Text style={styles.discountHintText}>
                  {Math.round(((parseFloat(form.comparePrice) - parseFloat(form.price)) / parseFloat(form.comparePrice)) * 100)}% discount
                </Text>
              </View>
            )}
          </View>
